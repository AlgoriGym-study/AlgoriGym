...
public class YHS {
	...
	static int[][] tunnel = {{},{0,1,2,3},{0,1},{2,3},{0,3},{1,3},{1,2},{0,2}}; //터널 번호에 따른 이동 가능 방향
	...
	static void bfs(int x, int y) {
		...
		while (!q.isEmpty()) {
			...
			if (curr.time == L) continue; //현재 칸까지의 이동시간이 L과 같다면 더이상 진행하지 않음 -> 큐가 더이상 추가되지 않으면서 자연스럽게 종료
			
			for (int num : tunnel[dir]) { //if 현재 칸이 1이라면 상하좌우로 이동 가능 -> tunnel[1] 의 인덱스들을 순회하며 각 방향으로 이동 가능한지 확인 (tunnel[1] = 0,1,2,3)
				...
				if (visited[nr][nc] || map[nr][nc] == 0) continue; //이미 방문했거나, 0이면 이동이 불가능하므로 스킵
				
				if (!canGo(curr.x, curr.y, nr, nc)) continue; //현재 칸과 다음 칸이 반드시 이어져있어야 하므로 비교
